...
export default {
  ...
  prompt: {
    ...
    claimTypeGuide: [
      ...
      "result — the outcome of a fight the subject fought in the last few days, reported as news. The subject's earlier win or loss mentioned in a profile, a preview, a photo caption, a highlights reel or clip, a recap, or another fighter's story (\"after beating the subject\", \"since losing to the subject\") is NOT a result: NO_CLAIM. A result gives the fight's date in facts.date (YYYY-MM-DD, or YYYY-MM if only the month is known): facts.date is the date the fight HAPPENED as stated in the text, never the article's publication date; when the text does not state when the fight happened, omit facts.date.",
