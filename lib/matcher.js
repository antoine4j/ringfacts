    ...
    properties: {
      ...
      claim: {
        ...
        properties: {
          ...
          facts: {
            type: "object",
            description: `Structured fields when present: ${P.factFields} facts.date is the date the fight HAPPENED as stated in the text, never the article's publication date; when the text does not state when the fight happened, omit facts.date.`,
          },
          sourcing: {
            type: "string",
...
- In the type rules below, NO_CLAIM means: write the fact, omit the claim (and, where it says so, role passing).
...
export async function matchItem({ subject, item, stories, confusables, subjectNames = null, fightWeekShape = true }) {
  // Fail before any request goes out: a caller still passing the old
  // `candidates` shape leaves `stories` undefined, and storyLines() would
  // happily render that as "(none)" and pay for a call that means nothing.
  if (!Array.isArray(stories)) {
    throw new TypeError("matchItem: stories must be an array (the shortlist of stories); callers still passing candidates need updating");
  }
