...
const VERDICT_TOOL = {
  ...
  input_schema: {
    ...
    required: ["verdict", "subject_role", "news_for_followers"],
  },
};
...
export async function matchItem({ subject, item, candidates, confusables, subjectNames = null }) {
  ...
  return normalizeVerdict(call?.input, new Set(candidates.map((c) => String(c.id))), { subjectNames, publishedAt: item.publishedAt });
}
