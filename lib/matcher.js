...
function buildPrompt({ fighter, item, candidates, confusables = P.confusables }) {
  ...
  const context = item.body
    ...
    : item.rssDescription
      ...
      : "";
  return `You match ${P.domainNoun} news articles to known claims about the ${P.subjectNoun} ${fighter}.
...
- WRONG_SUBJECT ONLY if the article is not about this ${P.subjectNoun} AT ALL: ${confusables}, or keyword-stuffed junk with no real connection. Judge on EVERYTHING shown: a body that clearly concerns ${fighter} overrides a headline that never names them.
- WRONG_SUBJECT also when ${fighter} appears NOWHERE in the headline or body excerpt shown — or only inside site furniture (a "LATEST NEWS"/related-articles link list, a photo caption, a navigation block). Such an article merely sits NEXT TO news about them; it is not about them.
- An article mainly about OTHER ${P.peerPlural} or the ${P.peerGroupNoun} whose OWN TEXT mentions or peripherally involves ${fighter} (as a rival, comparison, or context) is NO_CLAIM — NOT WRONG_SUBJECT. Readers still see NO_CLAIM articles; WRONG_SUBJECT articles are dropped.
- NO_CLAIM also covers articles asserting nothing claim-worthy about ${fighter}'s ${P.careerNoun} (${P.offTopicExamples}) — type ${domain.ignoredTypes.join("/")} articles are always NO_CLAIM.
