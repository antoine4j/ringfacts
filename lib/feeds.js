...
// case endings still match). The feed body is matched as reader-visible text,
// not raw HTML — a name inside an href slug or an image alt attribute is not
// a mention (Bloody Elbow's markup produced exactly those false positives).
export function matchesFighter(item, fighter) {
  const haystack =
    `${item.title} ${item.feedContent ? htmlToText(item.feedContent) : ""}`.toLowerCase();
