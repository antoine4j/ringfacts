...
//
// The thresholds here were MEASURED, not guessed (scripts/audit-digest-tier.js,
...
describe("digestTierFor", () => {
  ...
  // Reversed 2026-09-04: the headline used to outrank "passing", and for a
  // name-rich press that escape was the main door — every backdrop mention
  // kept a full headline. Now the role wins; the kill switch restores the
  // old order without a deploy. History: docs/decisions.md#tier-reorder
  test("a passing role outranks a name in the headline", () => {
    assert.equal(tier({ title: "Testov corners his teammate", body: bodyWith(0) }, "passing"), "tangential");
  });

  test("TIER_PASSING_OVERRIDES_HEADLINE=0 restores the headline escape", () => {
