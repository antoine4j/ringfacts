...
import { normalizeVerdict, buildPrompt, isStaleResult, hasAnnouncementFacts } from "./matcher.js";
...
describe("NEW", () => {
  ...
  test("every type the domain declares is accepted as-is (an announcement with a fight in it)", () => {
    for (const type of domain.claimTypes) {
      const facts = type === domain.ceremonyType ? { opponent: "Someone" } : {};
