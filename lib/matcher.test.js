...
import { normalizeVerdict, buildPrompt, isStaleResult } from "./matcher.js";
...
describe("a NEW claim must name the subject", () => {
  ...
  test("a canonical sentence about someone else is NO_CLAIM for this subject, role kept", () => {
    ...
    assert.deepEqual(v, { verdict: "NO_CLAIM", subject_role: "central" });
  });
...
describe("a result is news for two weeks, then history", () => {
  ...
  test("a June loss in an August article is NO_CLAIM, role kept", () => {
    assert.deepEqual(normalizeVerdict(result("2026-06-14"), offered, { publishedAt: published }), { verdict: "NO_CLAIM", subject_role: "central" });
