...
import { normalizeVerdict, buildPrompt, matchItem, isStaleResult, hasAnnouncementFacts, hasResultDate, VERDICT_TOOL } from "./matcher.js";
...
describe("the prompt", () => {
  test("the prompt lists stories most-similar first with their root title and member count, and marks booking claims", () => {
    ...
    assert.match(user, /\[7\][^\n]*BOOKING/);
    assert.match(system, /join only when the article reports the SAME news/);
    assert.match(system, /reaction when someone ELSE responds/);
  });
...
describe("the system block carries the claim-type discipline", () => {
  ...
  test("facts.date is pinned to the fight's own date, never the article's publication date", () => {
    assert.match(
      system,
      /facts\.date is the date the fight HAPPENED as stated in the text, never the article's publication date; when the text does not state when the fight happened, omit facts\.date\./,
    );
  });
...
describe("the four decisions", () => {
  ...
  test("reaction on a story that was not offered becomes new (an unoffered story is no story)", () => {
    const v = normalizeVerdict({ decision: "reaction", story_id: 99, fact: "Kawa answered Abdelaziz about Topuria.", subject_role: "supporting", news_for_followers: "yes" }, new Set(["7"]), { subjectNames: ["Topuria"] });
    assert.equal(v.decision, "new"); assert.equal(v.story_id, null); assert.equal(v.fact, "Kawa answered Abdelaziz about Topuria.");
  });
  ...
  test("wrong_subject with a stray story_id is WRONG_SUBJECT with no story_id", () => {
    const v = normalizeVerdict({ decision: "wrong_subject", story_id: 7, subject_role: "passing", news_for_followers: "no" }, new Set(["7"]));
    assert.equal(v.verdict, "WRONG_SUBJECT"); assert.equal(v.story_id, null);
  });
...
describe("matchItem rejects a caller still passing the old shape, before any request", () => {
  test("stories must be an array of the shortlist, not undefined candidates", async () => {
    await assert.rejects(
      matchItem({ subject: "Ilia Topuria", item: ITEM }),
      { message: "matchItem: stories must be an array (the shortlist of stories); callers still passing candidates need updating" },
    );
  });
});
