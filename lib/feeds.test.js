...
describe("matchesSubject", () => {
  ...
  // A second declension class, and the one that bit us for real: Ukrainian
  // -ко surnames inflect by REPLACING the final -о (Тестенка, Тестенку),
  // where -ов surnames only add to a stem the nominative already contains.
  // So a nominative -ко surname pasted straight into matchNames is
  // a stem that matches almost nothing — and, feeds being filtered before
  // storage, drops that subject's Ukrainian coverage leaving no row behind.
  // Fired on the live watchlist 2026-09-10: "Донченко" where "Донченк" was
  // meant. Both halves are asserted, because the bug is invisible from the
  // passing half alone: the nominative kept matching the whole time.
  describe("a -ко surname stem", () => {
    const KO_SUBJECT = { name: "Testenko Example", matchNames: ["Testenko", "Тестенк"] };
    const DECLINED = ["Тестенко", "Тестенка", "Тестенку", "Тестенком", "Тестенкові"];

    test("matches every declined form", () => {
      for (const form of DECLINED) {
        assert.equal(matchesSubject(item(`Новини про ${form} сьогодні`), KO_SUBJECT), true, form);
      }
    });

    test("but the nominative as a stem misses the cases that replace the -о", () => {
      const NAIVE = { name: "Testenko Example", matchNames: ["Testenko", "Тестенко"] };
      const missed = DECLINED.filter((form) => !matchesSubject(item(`Новини про ${form}`), NAIVE));
      // Not every case is missed, which is what makes this hard to spot: the
      // instrumental (-ом) and the dative -ові re-create the -о after the
      // stem, so they keep matching. Only -а and -у actually go dark.
      assert.deepEqual(missed, ["Тестенка", "Тестенку"]);
    });
  });
