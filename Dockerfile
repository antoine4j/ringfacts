...
COPY lib ./lib
COPY domain ./domain
# Glob, not a bare filename. The repo ships a real watchlist.js, but anyone who
# deletes it to run their own would otherwise hit a Docker error at build time;
# the glob matches whatever exists, so a missing watchlist surfaces at startup
