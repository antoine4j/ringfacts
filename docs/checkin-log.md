...
## 2026-09-06 morning (PST evening 09-05) — options B and D measured on a branch, nothing deployed (Anton present; Fable 5.1)

📊 **Data**: Anton asked for the recommended options measured before any change. Local branch `measure-story-matching` (not pushed, on his word). **B**: 167 of the 315 body-less articles fetched read-only (127 sites answer 403 — the production ceiling too); all 674 re-embedded on headline + body (TEST Gemini key, ~171k tokens, free tier, per-minute waits only). Bodies pull repeats AND connected stories closer; the best body threshold (all anchors 0.88: 235 held / 106 missed / 5 swallowed) equals option A — **B does not move the gate by itself**. **D** (`labels/measure-stories-llm.js`: stories as objects, top-3 by embedding, one Haiku call join/new/reaction, cascade): headline only 284 held / 211 placed right / 57 missed / 10 swallowed; **with bodies 307 / 244 / 34 / 10** against A's 247 / 97 / 94 / 5 and today's 173 / 106 / 168 / 7. On Anton's 612 rows alone: 279 of 304 held, 25 missed, 9 swallowed. The swallowed ten are mostly fight-week previews folded into the odds story. Shortlist recall 81% (65 repeats had their story outside the top three; Haiku then joins a wrong one 3 in 4). **Measured cost $1.74 per 674 articles** (1.09M in / 130k out), ≈ $2.30 a month; TEST key spend this session $3.28. Full tables: docs/grading/2026-09-06-story-matching.md.
🔧 **Changes**: labels/ gained fetch-bodies, embed-bodies, measure-stories-llm; export-stories carries text columns and the label author; measure-story-gate takes `--vectors`. 113 tests green. No pipeline change, no database write, no post. Bodies, vectors and 1348 verdicts kept in tmp/labels/ for the backfill if D is built.
💡 **Proposals for Anton**: (1) A now as a config change; (2) build D with B as its input — schema: `claims.reacts_to`, `claims.root_item`, `claims.decided_by`, backfill 72 stories from the labels, nothing dropped; (3) try a top-5 shortlist in the next run; (4) a reaction field in `feedback` so reactions can be scored.
👀 **Next attention**: Donchenko vs Soriano result must post within 6 h of the fight (Saturday noon PST); the 12 fight-week items still have no labels. Anton's decision on A / D.

## 2026-09-05 evening – 2026-09-06 ~06:00 UTC — the sheet is done, the feedback table is written, the story gate measured (Anton present until the review closed, then autonomous; Fable 5.1)

📊 **Data**: Anton closed the review: everything up to #107 by hand, then stories by root (a ruling on a root covers its members: 56 cells), then only the 39 held-but-real-news rows, the last 11 delegated to Claude. One Opus pass over all 674 headlines regrouped the stories (57 changes, 52 accepted as Claude rulings); three chained garbage-bag stories (#308, #16, #569) split by headline. **Section 3 done**: empty v1 `feedback` dropped, v2 applied, **1178 rows for 674 articles** (user 612, haiku 443, claude 109, sonnet 50), verified: no chains, no dup without a root, ten random rows match the sheet. New rulings in goals.md: a reply to a callout is 2; lifestyle stories are 2 (#208, #366); one interview is one story (#121, 24 members). **Story gate measured** (docs/grading/2026-09-05-story-gate.md): the root guard does not separate (a connected story sits as near the root as a repeat); today's rule holds 173 of 341 repeats and posts 168 again; all anchors at 0.85 holds 247 and swallows 5 useful stories vs 7 today — a config change, not a deploy. **Re-ingests diagnosed**: the embedding hold runs before URL decoding, so 206 embedding holds since Aug 9 have no resolved URL and no body (197 of the 315 headline-only review rows). Of 23 held-but-real-news rows, 12 were held by the matcher with a body in hand.
🔧 **Changes**: labels/ gained story rules (story-ruled cells, narrowed For-Anton table), export-stories, story-gate (+tests, 113 green), measure-story-gate; write-feedback keeps a graded post's 09-04 note when the cell is blank (#329, #345). TODO 3f measured, 3g (Eurosport links), 3h (decode before the early hold) added. No pipeline change.
💡 **Proposals for Anton**: (1) switch `DUP_ANCHORS_ALL=1` + `SEMANTIC_DUP_THRESHOLD=0.85` (measured better on both counts); (2) build 3h — decode before the early hold, bodies for held items; (3) put this week's bucket rulings into the matcher prompt (12 of 23 misses were matcher holds); (4) the 0.78–0.85 "same fact or a reaction?" question for the matcher.
👀 **Next attention**: Donchenko vs Soriano, Saturday noon PST — the result must post as a result line within 6 h; read the "because:" lines. The 12 fight-week items after the export have no labels yet.
---

## 2026-09-04 late evening – 2026-09-05 04:30 UTC — every archived article labelled; the sheet waits for Anton (Anton present at the start; Fable 5.1)

📊 **Data**: 674 archived articles (110 posted, 564 held) had labels in three places and 547 held ones had none. Plan agreed with Anton: grade first, he reviews one sheet, then one `feedback` table. Export of the evidence (`labels/export.js`, read-only, one SELECT) → 25 Haiku subagents read every held article against what the machine compared it with; 85 doubts (disagreements, low confidence, contradictions) re-read by Sonnet; 15 rulings by Claude on Anton's recorded rules. All on the subscription, $0 on the API keys. **Result: 23 held articles look like real news the group never saw** (bucket 2: Makhachev/Mendez/O'Malley assessing Topuria, his manager's rematch request, his recovery, two Donchenko interview segments, Amosov's next-opponent piece) and **the Friday weigh-in was swallowed**: four reports (items 601/617/618/625, 11:17–17:17 UTC) MATCHed the booking claim — all before the 18:01 fix; after it, the staredown (642) and the fight-night preview (653) posted. The dup gate's first pass agreed with the machine on 200/215; the wrong-subject hold on 146/148. No usable body: Mshale 55, Yahoo 33, Bloody Elbow 21 — the extraction task inherits this map.
🔧 **Changes** (committed, nothing deployed, nothing written to the database): `labels/` — export, groups, validate, doubts, derive-reason (Anton's 103 notes → reason codes: fine 29, junk 63, dup 8, old 3), sheet format/parser, build-review, anton-cell, write-feedback (drafted, not run); 60 unit tests, 374 total green. `docs/grading/2026-09-05-all-articles.md`: all 674 rows in id order with machine said / reviewer bucket / reason / dup of / why / Anton.
💡 **Proposals**: none until Anton has reviewed the sheet. The 23 misses are a G1 number for the month; whether the dup gate should let "the champion assessing him" through when the headline resembles an earlier post is his call.
👀 **Next attention**: **Anton reviews the sheet** ("as graded" or a correction in the last column). Then Section 3: drop the empty v1 `feedback` table, migrate v2, run `labels/write-feedback.js`. Saturday: the result must post as a 🕵️ result line; watch the `because:` lines.

---

## 2026-09-04 17:00–18:00 UTC (10:00–11:00 PDT) — fight week: the result would have been swallowed; fixed, deployed (Anton present; Fable 5.1)

📊 **Data**: the 17:17 UTC run (first hour on the new code) posted message 201, two Topuria items that were both the same story as message 200 an hour earlier — one fact, three claims (51/52/53), the headline embedding held three other rewrites at 0.81–0.82 and missed these two ("201 and 200 are the same news" — Anton). Donchenko fights Soriano at UFC Paris on Sat 2026-09-06, so a rehearsal on the test key fed his four real claims and five made-up fight-week articles to the matcher: **15/15 MATCH #49, the booking** — the result would have been held as "another sighting of the booking" and never posted. The Sonnet ceiling test on the 12 stubborn items: 6/12 vs Haiku 5/12 — the model is not the ceiling.
🔧 **Changes**: docs/decisions.md#fight-stages. A `reasoning` field first in the tool schema (the forced tool call gave the model no room to think; asked to reason in prose it got the result right on the same prompt), booking-type claims marked as bookings in the candidate list, a "stages of a fight are separate facts" rule with a worked example, and a gate demoting a `result` with no readable fight date to `other`. Rehearsal: 0 MATCH in 12, results NEW 6/8, the rest UNSURE (posts as a plain line). Tune split 37 → **38/45**, false loud claims **0**. The hunter logs a `because:` line under every verdict. Bench cost line now priced per model (the Sonnet run had been shown at Haiku prices). Dry run on the prod-copy branch clean. Deployed 18:01 UTC as image c6f6a0f2, no plain env vars, before the 18:17 run. Spent ≈ $1.6 on the test key this stretch.
💡 **Proposals**: the Topuria three-claims case is recorded (article-feedback, TODO 3c) with the ruling "a claim is an event, not an angle" — bench it after the fight week with a mint-time embedding guard. Claim-list cap (3d) still to do.
👀 **Next attention**: Friday's weigh-in is the first live test; Saturday night the result must post as a 🕵️ result line within the hour and later articles must MATCH it. Read the `because:` lines in the run logs.
